...
import { Visit } from '@/features/visits/types';
import {
  Dialog,
  DialogContent,
  DialogHeader,
...
export function VisitsList({ establishmentId }: VisitsListProps) {
  const [selectedVisit, setSelectedVisit] = useState<Visit | null>(null);
  const { data: visits = [], isLoading } = useQuery({
    queryKey: ['visits', establishmentId],
    queryFn: () => getVisitsByEstablishment(establishmentId),
  });

  if (isLoading) {
    return <div className="text-sm text-gray-600">Chargement des visites...</div>;
  }

  return (
    <div className="space-y-6">
      <div className="flex justify-between items-center">
        <h2 className="text-lg font-semibold">Visites planifiées</h2>
        <CreateVisitDialog establishmentId={establishmentId} />
      </div>
          ...
          {selectedVisit && (
            <div className="space-y-4">
              ...
              <div className="flex justify-end gap-2 mt-6">
                {selectedVisit.report_url ? (
                  <Button variant="outline" asChild>
                    <a href={selectedVisit.report_url} target="_blank" rel="noopener noreferrer">
                  ...
                  <Button variant="outline" asChild>
                    <Link to={`/visites/${selectedVisit.id}/rapport/${establishmentId}`}>
